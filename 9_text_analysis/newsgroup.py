...
# TfidVectorizer 객체를 tfidf_vect 객체명으로, LogisiticRegression 객체를 lr_clf 객체명으로 생성하는 Pipeline 생성
pipeline = Pipeline([
    ('tfidf_vect', TfidfVectorizer(stop_words='english', ngram_range=(1,2), max_df=300)),
    ('lr_clf', LogisticRegression(solver='saga', max_iter=100, C=10, random_state=156))
])
...
# GridSearchCV의 생성자에 Estimator가 아닌 Pipeline 객체 입력 (가벼운 튜닝: 샘플 일부 + 2-Fold)
_, X_tune_pipe, _, y_tune_pipe = train_test_split(
    X_train, y_train, train_size=3000, stratify=y_train, random_state=156
)
grid_cv_pipe = GridSearchCV(pipeline, param_grid=params, cv=2, scoring='accuracy')
grid_cv_pipe.fit(X_tune_pipe, y_tune_pipe)
print(grid_cv_pipe.best_params_, grid_cv_pipe.best_score_)
"""
{'lr_clf__C': 10, 'tfidf_vect__max_df': 300, 'tfidf_vect__ngram_range': (1, 2)} 
0.7210728891027183
"""

pipeline.set_params(**grid_cv_pipe.best_params_)
pipeline.fit(X_train, y_train)
